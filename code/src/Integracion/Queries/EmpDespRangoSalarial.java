...
public class EmpDespRangoSalarial implements Query<TEmpleado> {
	...
	public ArrayList<TEmpleado> execute(TQuery param) {
		...
		try{
			...
			while(rs.next()){
				lista.add(new TEmpleado(rs.getInt("idEmpleado"), rs.getString("nombre"), rs.getBoolean("activo"), rs.getFloat("sueldo"),
						rs.getString("dni"), rs.getString("horario"), rs.getString("idioma"), rs.getBoolean("titulo")));
			}

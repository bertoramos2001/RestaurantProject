...
public class SAMenuImp implements SAMenu { //TODO: revisar si esto esta bien
	...
	public int anadirProd(TMenuProducto tMenuProd) {
		...
		if (auxMenu != null && auxMenu.getActivo() && auxProd != null && auxProd.getActivo() && !daoMenuProd.contiene(tMenuProd)){
	...
	public ArrayList<TProducto> mostrarProdsPorMenu(Integer idMenu) {
		...
		Collection<TProducto> prodsLeidos = FactoriaIntegracion.getInstance().generarDAOMenuProd().leerProdsPorMenu(idMenu)
				, prodsActivos = new ArrayList<TProducto>();
